...
const HeroSection = () => {
  ...
  // Total frames extracted from the video (adjust this to match your actual frame count)
    ...
    const loadImages = async () => {
      ...
      for (let i = 1; i <= frameCount; i++) {
        const promise = new Promise<HTMLImageElement>((resolve, reject) => {
          const img = new Image();
          // Adjust padding matching ffmpeg output (e.g., frame_0001.webp)
          const paddedIndex = i.toString().padStart(4, "0");
          img.src = `/hero-video/frames/frame_${paddedIndex}.webp`;
      ...
      try {
        ...
        // Fallback to true so the UI is still interactive if an image fails
        setLoaded(true);
      }
    };
  ...
  useEffect(() => {
    ...
    // Helper function to draw the frame with "cover" styling logic
    ...
    const handleResize = () => {
      ...
      // Keep GSAP logic in sync after dimension changes
      ScrollTrigger.refresh();

      // Re-render current frame (it will be drawn instantly by ScrollTrigger anyway, but this is safe)
      renderFrame(0);
    };
    ...
    // Setup scroll-driven animation timeline
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        start: "top top",
        end: "+=300%", // The section will pin for 3 screen heights
        pin: true,     // Pin the hero section during playback
        scrub: 0.5,    // Smooth scrubbing
        anticipatePin: 1,
      }
    });

    // Tween the frame index over the duration of the scroll
    tl.to(frameObj, {
      frame: frameCount - 1,
      snap: "frame", // Optional: ensures we rest on a whole number
      ease: "none",
      onUpdate: () => {
        const frameIndex = Math.max(0, Math.min(images.length - 1, Math.round(frameObj.frame)));
    ...
    return () => {
      ...
      // Only kill this component's ScrollTrigger, not all triggers globally
    ...
    <section ref={containerRef} className="relative h-screen w-full bg-black overflow-hidden">
      ...
      {!loaded && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm transition-opacity duration-300">
          ...
          <p className="mt-4 text-primary-foreground font-body text-sm uppercase tracking-widest animate-pulse">
            Cargando {progress}%
          </p>
        </div>
      )}
      ...
      {/* Hero Content Layer */}
      <div className="relative z-10 section-padding h-full w-full flex flex-col justify-center pt-24 pb-16">
        <div className="w-full lg:w-1/2 flex flex-col items-start text-left">
          <motion.p
            className="body-sm mb-6 text-primary-foreground/80 font-medium"
          ...
          >
            Donde la
            <br />
            <span className="italic text-primary-foreground/90">madera</span>
            <br />
            cobra vida
          </motion.h1>

          <motion.p
            className="body-lg mt-8 max-w-md text-primary-foreground/90 leading-relaxed"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 1.1, duration: 0.6 }}
          >
            Sillas artesanales creadas con pasión, diseño contemporáneo y maderas nobles seleccionadas a mano.
          </motion.p>

          <motion.a
            href="#galeria"
            className="btn-hasann mt-10"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
